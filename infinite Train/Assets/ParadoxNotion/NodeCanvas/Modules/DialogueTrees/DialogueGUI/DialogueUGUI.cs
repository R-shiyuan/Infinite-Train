    ...
    public class DialogueUGUI : MonoBehaviour, IPointerClickHandler
    {
        ...
        void OnDialogueFinished(DialogueTree dlg)
        {
            ...
            if (cachedButtons != null)
            {
                ...
                {
                    if (tempBtn != null) Destroy(tempBtn.gameObject);
                }
                cachedButtons = null;
            }
            StopAllCoroutines();
        ...
        IEnumerator Internal_OnSubtitlesRequestInfo(SubtitlesRequestInfo info)
        {
            ...
            if (audio != null)
            {
                ...
                {
                    if (skipOnInput && anyKeyDown) { playSource.Stop(); break; }
            ...
            if (audio == null)
            {
                ...
                {
                    if (skipOnInput && inputDown) { actorSpeech.text = text; yield return null; break; }
                    ...
                    PlayTypeSound();
                    if (c == '.' || c == '!' || c == '?') { yield return StartCoroutine(DelayPrint(subtitleDelays.sentenceDelay)); PlayTypeSound(); }
                    if (c == ',') { yield return StartCoroutine(DelayPrint(subtitleDelays.commaDelay)); PlayTypeSound(); }
        ...
        {
            var timer = 0f;
            while (timer < time) { timer += Time.deltaTime; yield return null; }
        }
        ...
        void OnMultipleChoiceRequest(MultipleChoiceRequestInfo info)
        {
            ...
            if (info.showLastStatement) subtitlesGroup.gameObject.SetActive(true);
